...
void ReserveRegionAndCommit()
{
	...
	address = VirtualAlloc(address, regionsize, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
	if (address != NULL)
	{
		cout << "Allocation & commit succseffsul" << endl << "Address - " << std::dec << address << ".\n";;
	}
	else
