...
int main() {
	...
	wchar_t logfile_name[50] = L"log\\readers\\log_";
	...
	// INIT VIEW OF FILE
	shared_file_mapping_object = OpenFileMapping(FILE_MAP_READ, FALSE, L"Local\\SharedFile");

	view_of_file = MapViewOfFile(shared_file_mapping_object, FILE_MAP_READ, 0, 0, page_number * page_size);
	...
	for (int i = 0; i < page_number; ++i) {
		...
		reading_semaphores[i] = OpenSemaphore(SYNCHRONIZE, FALSE, semaphore_name_r[i]);
		writing_semaphores[i] = OpenSemaphore(SEMAPHORE_MODIFY_STATE, FALSE, semaphore_name_w[i]);
	...
	for (int i = 0; i < ITERATIONS_N; ++i) {
		...
		free_page_index = WaitForMultipleObjects(page_number, reading_semaphores, FALSE, INFINITE) - WAIT_OBJECT_0;
		...
		if (free_page_index >= 0 && free_page_index < page_number) {
			...
			ReleaseSemaphore(writing_semaphores[free_page_index], 1, NULL);
